import argparse
import torch
from src import NativelyRecursiveAgent, ResilientHAGModel, GeneralDataLoader
...
def main():
    ...
    if x is None:
        print(f"Error: Data for domain '{args.domain}' not found. Run 'make validate' to fetch it.")
        return

    # 2. Setup Agent with Resilient Model
    model = ResilientHAGModel(input_dim=16)
    agent = NativelyRecursiveAgent(base_model=model)

    # 3. Process Task
    print(f"Processing query: '{args.query}' on {len(x)} samples from {args.domain} domain...")
